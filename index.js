 ...
 100,000 requests/day&emsp;&emsp;&ensp;          1,000 requests/10 minutes`,
    ...
    try {
      ...
      if (originUrl.search.toLowerCase() != "?uri" && originUrl.search.toLowerCase()) {
        return new Response(`{error: "${Error2.name}", message: "Unable to decode given text '${originUrl.search.substring(1)}' into a URL. Is this a real website, and did you include 'http://' or 'https://'?"}`, {
          status: 404,
          statusText: Error2.statusText || "PageNotFound",
          headers: {"Content-Type": "text/plain"}
        });
      }
    }
    const originHeader = request.headers.get("Origin");
    if (targetUrl && isListedInWhitelist(targetUrl.href, blacklistUrls) || !isListedInWhitelist(originHeader, whitelistOrigins)) {
      return new Response(
        addHTML("blacklisted"),
        {
          status: 403,
